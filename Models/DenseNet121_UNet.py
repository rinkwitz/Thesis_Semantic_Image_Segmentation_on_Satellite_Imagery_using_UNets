...
class DenseNet121_UNet(torch.nn.Module):
    def __init__(self, in_channels, num_categories=2, deep_supervision=False, pretrained=True):
        ...
        # Encoder:
        self.enc1 = nn.ModuleList(list(densenet121(pretrained=self.pretrained).features)[:3])
        self.enc2 = nn.ModuleList(list(densenet121(pretrained=self.pretrained).features)[3:5])
        self.enc3 = nn.ModuleList(list(densenet121(pretrained=self.pretrained).features)[5:7])
        self.enc4 = nn.ModuleList(list(densenet121(pretrained=self.pretrained).features)[7:9])
        self.enc5 = nn.ModuleList(list(densenet121(pretrained=self.pretrained).features)[9:11])
...
if __name__ == '__main__':
    device = 'cuda:0'
    model = DenseNet121_UNet(in_channels=3, num_categories=2, deep_supervision=True)
    model.to(device)
