...
class vgg11_UNet(torch.nn.Module):
    def __init__(self, in_channels, num_categories=2, deep_supervision=False, pretrained=True):
        ...
        # Encoder:
        self.enc1 = nn.ModuleList(list(vgg11(pretrained=self.pretrained).features)[:2])
        self.enc2 = nn.ModuleList(list(vgg11(pretrained=self.pretrained).features)[2:5])
        self.enc3 = nn.ModuleList(list(vgg11(pretrained=self.pretrained).features)[5:10])
        self.enc4 = nn.ModuleList(list(vgg11(pretrained=self.pretrained).features)[10:15])
        self.enc5 = nn.ModuleList(list(vgg11(pretrained=self.pretrained).features)[15:20])
...
if __name__ == '__main__':
    device = 'cuda:0'
    model = vgg11_UNet(in_channels=3, num_categories=2, deep_supervision=True)
    model.to(device)
