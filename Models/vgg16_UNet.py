...
class vgg16_UNet(torch.nn.Module):
    def __init__(self, in_channels, num_categories=2, deep_supervision=False, pretrained=True):
        ...
        self.main_model_name = 'vgg16_unet'

        # Encoder:
        self.enc1 = nn.ModuleList(list(vgg16(pretrained=self.pretrained).features)[:4])
        self.enc2 = nn.ModuleList(list(vgg16(pretrained=self.pretrained).features)[4:9])
        self.enc3 = nn.ModuleList(list(vgg16(pretrained=self.pretrained).features)[9:16])
        self.enc4 = nn.ModuleList(list(vgg16(pretrained=self.pretrained).features)[16:23])
        self.enc5 = nn.ModuleList(list(vgg16(pretrained=self.pretrained).features)[23:30])

        # Decoder:
        self.dec4_UpConcat2d = UpConcat2d(512, 512)
        self.dec4_Conv2dBlock = Conv2dBlock(1024, 512, True)
        self.dec3_UpConcat2d = UpConcat2d(512, 256)
        self.dec3_Conv2dBlock = Conv2dBlock(512, 256, True)
    ...
    def initialize_decoder_weights(self):
        for layer in [self.dec4_UpConcat2d, self.dec4_Conv2dBlock, self.dec3_UpConcat2d, self.dec3_Conv2dBlock,
                      self.dec2_UpConcat2d, self.dec2_Conv2dBlock, self.dec1_UpConcat2d, self.dec1_Conv2dBlock]:
...
if __name__ == '__main__':
    device = 'cuda:0'
    model = vgg16_UNet(in_channels=3, num_categories=2, deep_supervision=True)
    model.to(device)
