...
model_path = Path('experiments/experiment_4/scag_unet_32-512_wDeepSupervision_192.pt')
...
tensor_paths_spatial = [p for p in Path('tmp').iterdir() if 'scag-attention_spatial_' in p.name]
...
# channel:
fig = plt.figure(figsize=(20, 10), dpi=100)
tensor_paths_channel = [p for p in Path('tmp').iterdir() if 'scag-attention_channel_' in p.name]
